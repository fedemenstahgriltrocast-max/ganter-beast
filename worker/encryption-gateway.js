...
import { encryptString, decryptString } from '../encryption/aes-gcm.js';
...
export default {
  async fetch(request, env) {
    ...
    if (!masterSecret) {
      return cors(json({ ok: false, error: 'misconfigured_secret' }, 500), origin);
    }
    ...
    try {
      if (method === 'GET' && url.pathname === '/') {
        return cors(json({
          ...
          compliance: {
            ...
            policies: ['NIST CSF PR.DS', 'CISA Cyber Essentials', 'PCI DSS Req.3']
          }
        }), origin);
      }
      ...
      if (method === 'POST' && url.pathname === '/encrypt') {
        ...
        if (!plaintext) {
          return cors(json({ ok: false, error: 'invalid_plaintext' }, 400), origin);
        }

        const associatedData = typeof body?.aad === 'string' ? body.aad.slice(0, 128) : undefined;
